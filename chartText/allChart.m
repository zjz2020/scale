...
- (void)drawHorizontal{
    ...
    for (NSInteger i = 0; i <= kYEqualPaths; i++) {
        ...
        [path addLineToPoint:CGPointMake(chartAllStartX + _xValues.count * 50, chartAllAxisSpanY * i + kTopSpace)];
        [path closePath];
...
- (void)loadline:(NSInteger)lineY WithShapeLayer:(CAShapeLayer *)ShapeLayer{
    ...
    //sets the curve
    if (self.curve) {
        bezierLine =[bezierLine smoothedPathWithGranularity:10];
    }
    //Graphics trajectory
